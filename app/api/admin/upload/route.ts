...
const MAX_MULTIPART_SIZE = 6 * 1024 * 1024;
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Never trust the browser-supplied filename. UUID-only object names prevent
    // path traversal, collisions, and content-type confusion.
    const objectPath = `uploads/${randomUUID()}.${type.ext}`;
    ...
    if (uploadError) {
      logServerError("Product image upload failed", uploadError);
      return NextResponse.json({ error: "Failed to upload image" }, { status: 500 });
    }

    const { data } = supabase.storage.from(BUCKET).getPublicUrl(objectPath);
    if (!data.publicUrl) {
      // Best-effort cleanup if the bucket URL cannot be generated.
    ...
    logServerError("Product image upload request failed", err);
